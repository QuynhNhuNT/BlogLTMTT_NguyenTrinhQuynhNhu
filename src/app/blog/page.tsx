...
export default function BlogPage() {
  ...
  return (
    <div className="min-h-screen bg-pink-50">
      <div className="container max-w-[1600px] py-8 lg:py-16">
      <PageHeader
        title="Blog"
      ...
      <div className="my-8">
        <div className="relative max-w-2xl mx-auto">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-4 text-lg border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none"
          />
        </div>
      </div>

      <hr className="my-12 border-gray-200 border-2" />
        ...
        <div className="space-y-16">
          {categorizedBlogs.map(([category, blogs]) => (
            <div key={category} className="space-y-8">
              ...
              {/* Blog Posts Grid */}
              <div className="grid gap-12 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3">
                {blogs.map((blog) => (
                  <article
                    key={blog.slug}
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col p-10">
                      {blog.date && (
                        <p className="mb-4 text-base font-semibold text-pink-600">
                          {formatDate(blog.date)}
                        </p>
                      )}

                      <h2 className="mb-6 text-3xl font-bold leading-tight text-gray-900 group-hover:text-pink-600 transition-colors lg:text-4xl">
                        {blog.title}
                      </h2>
                      
                      {blog.description && (
                        <p className="mb-8 flex-1 text-lg leading-relaxed text-gray-700 line-clamp-5 lg:text-xl">
                          {blog.description}
                        </p>
                      )}
                      ...
                      <Link 
                        href={blog.slug} 
                        className="mt-auto inline-flex items-center text-lg font-bold text-pink-600 hover:text-pink-700 transition-colors"
                      >
                        Đọc thêm
                        <span className="ml-3 text-2xl transition-transform group-hover:translate-x-2">→</span>
                      </Link>
                    </div>
                  </article>
                ))}
